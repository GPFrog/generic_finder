defmodule GenericFinderServerWeb.PageController do
  ...
  def medicineLookup(conn, %{"name" => name, "activeingredient" => activeingredient, "company" => company, "symptom" => symptom, "si_do" => si_do, "si_gun_gu" => si_gun_gu})do
    json conn, GenericFinderServer.Test.start(name, activeingredient, company, symptom, si_do, si_gun_gu, :medicineLookup)
  end
  ...
  def medicinePriceDelete(conn, %{"email" => email, "date" => date, "bussiness_number" => bussiness_number, "medicine_name" => medicine_name, "price" => price})do
    json conn, GenericFinderServer.Test.start(email, date, bussiness_number, medicine_name, price, :medicinePriceDelete)
  end
